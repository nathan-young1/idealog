...
class AddTasksToExistingIdea extends StatefulWidget {
  final Idea idea;
  final bool? thisPageWasOpenedFromTheMainMenu;

  AddTasksToExistingIdea({Key? key, required this.idea, this.thisPageWasOpenedFromTheMainMenu}) : super(key: key);
...
class _AddTasksToExistingIdeaState extends State<AddTasksToExistingIdea> {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
        child: WillPopScope(
          onWillPop: () async {
            (widget.thisPageWasOpenedFromTheMainMenu != null)
              ?await Navigator.of(context).push(PageTransition(child: MenuPageView(), type: PageTransitionType.leftToRightWithFade))
              :Navigator.pop(context);
            
            // stop the willPopScope from handling back pressed events by itself.
            return false;
          },
          child: Scaffold(
            body: Padding(
                ...
                Container(
                 ...
                 child: Column(
                   ...
                   children: [
                     ...
                     iconSize: 37,
                     onPressed: () async => (widget.thisPageWasOpenedFromTheMainMenu != null)
                      ?await Navigator.of(context).push(PageTransition(child: MenuPageView(), type: PageTransitionType.leftToRightWithFade))
                      :Navigator.pop(context)
                      ),
                      Expanded(
            ...
            bottomNavigationBar: GestureDetector(
                onTap: () async {
                  ...
                  // tell the detail page that is has the main page page still alive in the navigation stack so that it does pop's the current context.
                  await Navigator.of(context).push(PageTransition(child: IdeaDetail(idea: widget.idea), type: PageTransitionType.leftToRightWithFade));
                  },
                child: Container(
                  height: 65,
                  color: DarkBlue,
